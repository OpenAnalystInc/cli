...
Scope: read-only comparison between the original TypeScript source at `/home/bellman/Workspace/openanalyst-cli/src/` and the Rust port under `rust/crates/`.
...
- Hook config can be inspected via Rust config reporting in `rust/crates/commands/src/lib.rs` and `rust/crates/openanalyst-cli/src/main.rs`.
...
## skills/ and OPENANALYST.md discovery
...
- OPENANALYST.md discovery is implemented in `rust/crates/runtime/src/prompt.rs`.
- Rust supports `/memory` and `/init` via `rust/crates/commands/src/lib.rs` and `rust/crates/openanalyst-cli/src/main.rs`.
...
- Main CLI/repl/prompt handling lives in `rust/crates/openanalyst-cli/src/main.rs`.
...
- Stream/tool event translation in `rust/crates/openanalyst-cli/src/main.rs`.
...
### Rust exists
Evidence:
- Core OpenAnalyst API client in `rust/crates/api/src/{client,error,sse,types}.rs`.
...
- **Prompt mode tools enabled**
  - `rust/crates/openanalyst-cli/src/main.rs` now constructs prompt mode with `LiveCli::new(model, true, ...)`.
- **Default permission mode = DangerFullAccess**
  - Runtime default now resolves to `DangerFullAccess` in `rust/crates/openanalyst-cli/src/main.rs`.
  - Clap default also uses `DangerFullAccess` in `rust/crates/openanalyst-cli/src/args.rs`.
  - Init template writes `dontAsk` in `rust/crates/openanalyst-cli/src/init.rs`.
- **Streaming `{}` tool-input prefix bug**
  - `rust/crates/openanalyst-cli/src/main.rs` now strips the initial empty object only for streaming tool input, while preserving legitimate `{}` in non-stream responses.
