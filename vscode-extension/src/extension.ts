...
import {
  ...
  IDE_DEFINITIONS,
  type IdeInfo,
} from '@google/openanalyst-cli-core/src/ide/detect-ide.js';

const CLI_IDE_COMPANION_IDENTIFIER = 'Google.openanalyst-cli-vscode-ide-companion';
const INFO_MESSAGE_SHOWN_KEY = 'openanalystCliInfoMessageShown';
export const DIFF_SCHEME = 'openanalyst-diff';
  ...
  try {
    ...
    if (
      ...
      const selection = await vscode.window.showInformationMessage(
        `A new version (${latestVersion}) of the OpenAnalyst CLI Companion extension is available.`,
...
export async function activate(context: vscode.ExtensionContext) {
  logger = vscode.window.createOutputChannel('OpenAnalyst CLI IDE Companion');
  ...
  if (
    ...
    void vscode.window.showInformationMessage(
      'OpenAnalyst CLI Companion extension successfully installed.',
    );
  ...
  context.subscriptions.push(
    ...
    vscode.commands.registerCommand('openanalyst-cli.runOpenAnalystCLI', async () => {
      const workspaceFolders = vscode.workspace.workspaceFolders;
      if (!workspaceFolders || workspaceFolders.length === 0) {
        vscode.window.showInformationMessage(
          'No folder open. Please open a folder to run OpenAnalyst CLI.',
        );
        return;
      }
      ...
      if (workspaceFolders.length === 1) {
        ...
        selectedFolder = await vscode.window.showWorkspaceFolderPick({
          placeHolder: 'Select a folder to run OpenAnalyst CLI in',
        });
      }

      if (selectedFolder) {
        const openanalystCmd = 'openanalyst';
        const terminal = vscode.window.createTerminal({
          name: `OpenAnalyst CLI (${selectedFolder.name})`,
          cwd: selectedFolder.uri.fsPath,
        });
        terminal.show();
        terminal.sendText(openanalystCmd);
      }
    }),
    vscode.commands.registerCommand('openanalyst-cli.showNotices', async () => {
