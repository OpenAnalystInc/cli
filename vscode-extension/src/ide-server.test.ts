...
vi.mock('@google/openanalyst-cli-core', async (importOriginal) => {
  const actual =
    await importOriginal<typeof import('@google/openanalyst-cli-core')>();
  return {
    ...actual,
    tmpdir: vi.fn(() => '/tmp'),
  };
...
const getPortFromMock = (
  ...
  const port = vi
    .mocked(replaceMock)
    .mock.calls.find((call) => call[0] === 'OPENANALYST_CLI_IDE_SERVER_PORT')?.[1];
...
describe('IDEServer', () => {
  ...
  it('should set environment variables and workspace path on start with multiple folders', async () => {
    ...
    expect(replaceMock).toHaveBeenNthCalledWith(
      1,
      'OPENANALYST_CLI_IDE_SERVER_PORT',
    ...
    expect(replaceMock).toHaveBeenNthCalledWith(
      2,
      'OPENANALYST_CLI_IDE_WORKSPACE_PATH',
      expectedWorkspacePaths,
    );

    expect(replaceMock).toHaveBeenNthCalledWith(
      3,
      'OPENANALYST_CLI_IDE_AUTH_TOKEN',
      'test-auth-token',
    );
    ...
    const expectedPortFile = path.join(
      ...
      'openanalyst',
      'ide',
      `openanalyst-ide-server-${process.ppid}-${port}.json`,
    );
    ...
    expect(fs.mkdir).toHaveBeenCalledWith(path.join('/tmp', 'openanalyst', 'ide'), {
      recursive: true,
    });
