...
vi.mock('@google/openanalyst-cli-core/src/ide/detect-ide.js', async () => {
  const actual = await vi.importActual(
    '@google/openanalyst-cli-core/src/ide/detect-ide.js',
  );
  return {
    ...actual,
...
describe('activate', () => {
  ...
  it('should launch the OpenAnalyst CLI when the user clicks the button', async () => {
    const showInformationMessageMock = vi
      .mocked(vscode.window.showInformationMessage)
      .mockResolvedValue('Re-launch OpenAnalyst CLI' as never);
    vi.mocked(context.globalState.get).mockReturnValue(undefined);
    vi.mocked(vscode.extensions.getExtension).mockReturnValue({
      packageJSON: { version: '1.1.0' },
    } as vscode.Extension<unknown>);
  ...
  describe('update notification', () => {
    ...
    it('should show an update notification if a newer version is available', async () => {
      ...
      expect(showInformationMessageMock).toHaveBeenCalledWith(
        'A new version (1.2.0) of the OpenAnalyst CLI Companion extension is available.',
    ...
    it('should execute the install command when the user clicks "Update"', async () => {
      ...
      expect(executeCommandMock).toHaveBeenCalledWith(
        ...
        'Google.openanalyst-cli-vscode-ide-companion',
      );
    });
