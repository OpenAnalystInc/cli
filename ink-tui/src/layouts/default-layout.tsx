...
export function DefaultLayout(): React.ReactElement {
  ...
  const handleGlobalKeys = useCallback(
    (input: string, key: { ctrl?: boolean; escape?: boolean; meta?: boolean; shift?: boolean }, command: Command | undefined): boolean => {
      ...
      if (command === Command.RUN_IN_BACKGROUND) return false;
  ...
  return (
    <Box flexDirection="column" width={terminal.width}>
      {/* ── Main content: chat + optional sidebar column ── */}
      <Box flexDirection="row">
        {/* Chat panel — takes remaining width, height is content-driven */}
        <Box flexDirection="column" flexGrow={1} minWidth={30}>
          <ChatPanel messages={messages} />
        </Box>

        {/* Sidebar — right column, only when visible (F2 toggle) */}
        {showSidebar && (
          <Box
            width={SIDEBAR_WIDTH}
            flexShrink={0}
            ...
            borderStyle="round"
            borderColor={uiState.sidebarFocused ? colors.status.warning : colors.border.default}
            overflow="hidden"
          >
            <Sidebar
              agents={uiState.sidebarAgents}
              files={uiState.sidebarFiles}
              plans={uiState.sidebarPlans}
              routing={uiState.sidebarRouting}
              activity={uiState.sidebarActivity}
      ...
      {/* ── Toast notification (auto-hiding) ── */}
