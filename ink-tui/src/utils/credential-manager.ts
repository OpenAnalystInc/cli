...
export const PROVIDER_CONFIG: Record<string, ProviderConfig> = {
  openai: {
    ...
    creditParser: (r: unknown) => {
      const data = r as { total_available?: number };
  ...
  openrouter: {
    ...
    creditAuthHeader: 'bearer',
    creditParser: (r: unknown) => {
      const data = r as { data?: { limit_remaining?: number } };
  ...
  stability: {
    ...
    creditAuthHeader: 'bearer',
    creditParser: (r: unknown) => {
      const data = r as { credits?: number };
...
// ---------------------------------------------------------------------------
// SQLite wrapper (sql.js -- pure JS, no native compilation)
// ---------------------------------------------------------------------------
 ...
 */
async function getSqlDb(): Promise<SqlJsDatabase | null> {
  if (_sqlDb) return _sqlDb;
  if (_sqlInitAttempted) return null;
  ...
  try {
    ...
    if (fs.existsSync(dbPath)) {
      ...
      db = new SQL.Database(buffer) as unknown as SqlJsDatabase;
    } else {
...
class CredentialManager {
  ...
  async saveApiKey(provider: string, apiKey: string): Promise<void> {
    ...
    try {
      ...
      if (db) {
        db.run(
          `INSERT OR REPLACE INTO credentials (provider, api_key, env_var_name, saved_at, source)
           VALUES (?, ?, ?, ?, ?)`,
          [provider, apiKey, config.envVar, timestamp, 'login'],
        );
        persistSqlDb();
      }
    } catch {
  ...
  getApiKey(provider: string): ResolvedKey {
    const config = PROVIDER_CONFIG[provider];
    if (!config) return { key: null, source: null };
    ...
    if (_sqlDb) {
      try {
        ...
        if (rows.length > 0 && rows[0]!.values.length > 0) {
          const key = rows[0]!.values[0]![0] as string;
  ...
  getApiKeyByEnvVar(envVar: string): ResolvedKey {
    const provider = Object.entries(PROVIDER_CONFIG).find(
      ([, cfg]) => cfg.envVar === envVar,
    );
  ...
  async removeAll(): Promise<void> {
    for (const providerKey of Object.keys(PROVIDER_CONFIG)) {
      if (providerKey === 'gemini') continue; // Keep Gemini OAuth
   ...
   */
  async fetchCredits(provider: string): Promise<string> {
    const config = PROVIDER_CONFIG[provider];
    if (!config) return 'Unknown provider';

    const resolved = this.getApiKey(provider);
    if (!resolved.key) return 'No API key';
    ...
    try {
      const headers: Record<string, string> =
        config.creditAuthHeader === 'x-api-key'
   ...
   */
  async fetchAllCredits(): Promise<Record<string, string>> {
  ...
  detectProvider(apiKey: string): string | null {
    ...
    if (apiKey.startsWith('sk-oa-')) return 'openanalyst';
  ...
  private upsertEnvKey(envPath: string, key: string, value: string): void {
    try {
  ...
  private removeEnvKey(envPath: string, key: string): void {
    try {
  ...
  private saveToCredJson(
    ...
    try {
      ...
      if (fs.existsSync(this.credJsonPath)) {
        try {
          creds = JSON.parse(
            fs.readFileSync(this.credJsonPath, 'utf-8'),
      ...
      (creds['providers'] as Record<string, unknown>)[config.displayName] = {
  ...
  private removeFromCredJson(provider: string): void {
    try {
      if (!fs.existsSync(this.credJsonPath)) return;
      ...
      const providers = creds['providers'] as
  ...
  private async validateKey(
    ...
    if (!url) return `${config.displayName} configured`;
