...
function getPermissionDisplay(mode, colors) {
    switch (mode) {
        case 'read-only':
...
export function SidebarActivity({ activity, isFocused, colors, permissionMode, elapsedSecs = 0, totalTokens = 0, }) {
    ...
    return (_jsxs(Box, { flexDirection: "column", children: [_jsxs(Box, { children: [_jsxs(Text, { color: colors.text.accent, children: [" ", '\u21C5', " "] }), _jsxs(Text, { color: textColor, children: [activity.toolCallCount, " tool calls"] })] }), _jsxs(Box, { children: [_jsxs(Text, { color: colors.status.done, children: [" ", '\u2193', " "] }), _jsxs(Text, { color: textColor, children: [formatTokens(totalTokens), " tokens"] })] }), _jsxs(Box, { children: [_jsxs(Text, { color: colors.status.warning, children: [" ", '\u2299', " "] }), _jsxs(Text, { color: textColor, children: [formatElapsed(elapsedSecs), " elapsed"] })] }), _jsxs(Box, { children: [_jsxs(Text, { color: perm.color, children: [" ", perm.icon, " "] }), _jsxs(Text, { color: textColor, children: ["mode: ", perm.label] })] }), (() => {
                const dp = providerPreferences.getDefaultProvider();
                const dpConfig = dp ? PROVIDER_CONFIG[dp] : null;
                if (dpConfig) {
                    return (_jsxs(Box, { children: [_jsxs(Text, { color: colors.status.warning, children: [' \u2605', " "] }), _jsx(Text, { color: textColor, children: dpConfig.displayName })] }));
                }
                return null;
