...
// ---------------------------------------------------------------------------
const MessageItem = React.memo(function MessageItem({ message, isFocused, }) {
    const terminal = useTerminal();
    const chatActions = useChatActions();
    const credits = useCredits();
    switch (message.kind) {
        case 'user':
            return (_jsx(UserMessage, { text: message.text, isSlashCommand: message.isSlashCommand, isFocused: isFocused }));
        case 'assistant':
            return (_jsx(AssistantMessage, { content: message.content, streaming: message.streaming, isFocused: isFocused }));
        case 'system':
        ...
        case 'file_output':
            return (_jsx(FileOutput, { fileType: message.fileType, description: message.description, filePath: message.filePath, isFocused: isFocused }));
        case 'banner':
            return (_jsx(Banner, { version: message.version, username: message.displayName, email: message.email, org: message.organization, workingDir: message.workingDir, provider: message.provider, modelDisplay: message.modelDisplay, credits: credits.balance !== 'checking...' && credits.balance !== 'API credits'
                    ? (credits.provider !== 'unknown' ? `${credits.provider}: ${credits.balance}` : credits.balance)
                    : (message.credits || credits.balance), tips: [...message.tips], terminalWidth: terminal.width }));
        default:
            return null;
    }
});
...
// Component
// ---------------------------------------------------------------------------
export const MessageList = React.memo(function MessageList({ messages, focusedIndex, }) {
    const { colors } = useTheme();
    return (_jsx(Box, { flexDirection: "column", children: messages.map((msg, idx) => {
            const isFocused = idx === focusedIndex;
            return (_jsx(Box, { flexDirection: "column", ...(isFocused
                    ? { borderStyle: 'single', borderColor: colors.border.focus, borderLeft: true, borderRight: false, borderTop: false, borderBottom: false }
                    : {}), children: _jsx(MessageItem, { message: msg, isFocused: isFocused }) }, msg.id));
        }) }));
});
