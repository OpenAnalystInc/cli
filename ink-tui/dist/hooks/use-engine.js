...
export function useEngine(config = {}, handlers = {}) {
    ...
    const mockEmitterRef = useRef(null);
    ...
    const spawnEngine = useCallback(() => {
        ...
        if (cfg.mock)
            return; // Don't spawn in mock mode
...
function handleMockAction(action, dispatch) {
    switch (action.type) {
        case 'submit_prompt': {
 ...
 */
function simulateResponse(prompt, dispatch) {
...
export function createMockEngine() {
    ...
    return {
        ...
        send(action) {
            handleMockAction(action, dispatch);
        },
        dispose() {
            handlers.clear();
