...
// ---------------------------------------------------------------------------
export const defaultKeyBindings = new Map([
    // ===== Global =====
    [Command.QUIT, [
 ...
 */
export function matchesCommand(command, input, key, config = defaultKeyBindings) {
 ...
 */
export function resolveCommand(input, key, candidates, config = defaultKeyBindings) {
...
export function formatKeyBinding(binding) {
    ...
    if (binding.shift)
        parts.push('Shift');
    const displayName = KEY_DISPLAY_MAP[binding.name] ??
 ...
 */
export function formatCommand(command, config = defaultKeyBindings) {
    const bindings = config.get(command);
