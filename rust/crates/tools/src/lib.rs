...
pub fn mvp_tool_specs() -> Vec<ToolSpec> {
    vec![
        ...
        ToolSpec {
            name: "Config",
            description: "Get or set OpenAnalyst settings.",
            input_schema: json!({
                "type": "object",
                "properties": {
                    "setting": { "type": "string" },
...
fn execute_knowledge(input: KnowledgeInput) -> Result<KnowledgeOutput, String> {
    ...
    // Check for API key — KB server requires oa_ prefix keys
    // Priority: OPENANALYST_KB_KEY (dedicated KB key) > OA_API_KEY > OPENANALYST_API_KEY > OPENANALYST_AUTH_TOKEN
    let api_key = std::env::var("OPENANALYST_KB_KEY")
        .or_else(|_| std::env::var("OA_API_KEY"))
        .or_else(|_| std::env::var("OPENANALYST_API_KEY"))
...
fn build_http_client() -> Result<Client, String> {
    Client::builder()
        ...
        .user_agent("openanalyst-tools/0.1")
        .build()
        .map_err(|error| error.to_string())
}
...
fn build_search_url(query: &str) -> Result<reqwest::Url, String> {
    if let Ok(base) = std::env::var("OPENANALYST_WEB_SEARCH_BASE_URL") {
...
fn todo_store_path() -> Result<std::path::PathBuf, String> {
    if let Ok(path) = std::env::var("OPENANALYST_TODO_STORE") {
...
fn spawn_agent_job(job: AgentJob) -> Result<(), String> {
    let thread_name = format!("openanalyst-agent-{}", job.manifest.agent_id);
...
fn agent_store_dir() -> Result<std::path::PathBuf, String> {
    if let Ok(path) = std::env::var("OPENANALYST_AGENT_STORE") {
        return Ok(std::path::PathBuf::from(path));
    }
    let cwd = std::env::current_dir().map_err(|error| error.to_string())?;
    if let Some(workspace_root) = cwd.ancestors().nth(2) {
        return Ok(workspace_root.join(".openanalyst-agents"));
    }
    Ok(cwd.join(".openanalyst-agents"))
}
...
fn normalize_subagent_type(subagent_type: Option<&str>) -> String {
    ...
    match canonical_tool_token(trimmed).as_str() {
        ...
        "openanalystguide" | "openanalystguideagent" | "guide" => String::from("openanalyst-guide"),
    ...
    let cell_id = match edit_mode {
        NotebookEditMode::Insert => {
            let resolved_cell_type = resolved_cell_type
                .ok_or_else(|| String::from("Insert requires a cell type"))?;
        ...
        NotebookEditMode::Delete => {
            let idx = target_index.ok_or_else(|| String::from("Delete requires a target cell index"))?;
            if idx >= cells.len() {
                return Err(format!("Cell index {idx} out of range (notebook has {} cells)", cells.len()));
            }
        ...
        NotebookEditMode::Replace => {
            let resolved_cell_type = resolved_cell_type
                .ok_or_else(|| String::from("Replace requires a cell type"))?;
            let idx = target_index.ok_or_else(|| String::from("Replace requires a target cell index"))?;
            if idx >= cells.len() {
                return Err(format!("Cell index {idx} out of range (notebook has {} cells)", cells.len()));
            }
...
fn config_file_for_scope(scope: ConfigScope) -> Result<PathBuf, String> {
    ...
    Ok(match scope {
        ...
        ConfigScope::Settings => cwd.join(".openanalyst").join("settings.local.json"),
    })
}

fn config_home_dir() -> Result<PathBuf, String> {
    if let Ok(path) = std::env::var("OPENANALYST_CONFIG_HOME") {
...
mod tests {
    ...
    fn temp_path(name: &str) -> PathBuf {
        let unique = std::time::SystemTime::now()
            ...
            .expect("time")
            .as_nanos();
        std::env::temp_dir().join(format!("openanalyst-tools-{unique}-{name}"))
    }
    ...
    fn web_search_extracts_and_filters_results() {
        ...
        std::env::remove_var("OPENANALYST_WEB_SEARCH_BASE_URL");
    ...
    fn web_search_handles_generic_links_and_invalid_base_url() {
        ...
        std::env::set_var("OPENANALYST_WEB_SEARCH_BASE_URL", "://bad-base-url");
    ...
    fn todo_write_persists_and_returns_previous_state() {
        ...
        std::env::set_var("OPENANALYST_TODO_STORE", &path);
        ...
        .expect("TodoWrite should succeed");
        std::env::remove_var("OPENANALYST_TODO_STORE");
    ...
    fn agent_persists_handoff_metadata() {
        ...
        std::env::set_var("OPENANALYST_AGENT_STORE", &dir);
        ...
        std::env::remove_var("OPENANALYST_AGENT_STORE");
  ...
  "nbformat_minor": 5
    ...
    fn config_reads_and_writes_supported_values() {
        ...
        std::fs::create_dir_all(home.join(".openanalyst")).expect("home dir");
        std::fs::create_dir_all(cwd.join(".openanalyst")).expect("cwd dir");
        std::fs::write(
            home.join(".openanalyst").join("settings.json"),
            r#"{"verbose":false}"#,
        )
        .expect("write global settings");

        let original_home = std::env::var("HOME").ok();
        let original_config_home = std::env::var("OPENANALYST_CONFIG_HOME").ok();
        let original_dir = std::env::current_dir().expect("cwd");
        ...
        match original_config_home {
            Some(value) => std::env::set_var("OPENANALYST_CONFIG_HOME", value),
            None => std::env::remove_var("OPENANALYST_CONFIG_HOME"),
        }
