...
fn render_project_context(project_context: &ProjectContext) -> String {
    ...
    if !project_context.instruction_files.is_empty() {
        bullets.push(format!(
            "OpenAnalyst instruction files discovered: {}.",
...
fn render_instruction_files(files: &[ContextFile]) -> String {
    let mut sections = vec!["# OpenAnalyst instructions".to_string()];
...
fn render_config_section(config: &RuntimeConfig) -> String {
    ...
    if config.loaded_entries().is_empty() {
        lines.extend(prepend_bullets(vec![
            "No OpenAnalyst settings files loaded.".to_string()
        ]));
        return lines.join("\n");
    }
...
mod tests {
    ...
    fn discovers_instruction_files_from_ancestor_chain() {
        ...
        fs::create_dir_all(nested.join(".openanalyst")).expect("nested openanalyst dir");
        fs::write(root.join("OPENANALYST.md"), "root instructions").expect("write root instructions");
        fs::write(root.join("OPENANALYST.local.md"), "local instructions")
            .expect("write local instructions");
        fs::create_dir_all(root.join("apps")).expect("apps dir");
        fs::create_dir_all(root.join("apps").join(".openanalyst")).expect("apps openanalyst dir");
        fs::write(root.join("apps").join("OPENANALYST.md"), "apps instructions")
            .expect("write apps instructions");
        fs::write(
            root.join("apps").join(".openanalyst").join("instructions.md"),
            "apps dot openanalyst instructions",
        )
        .expect("write apps dot openanalyst instructions");
        fs::write(nested.join(".openanalyst").join("OPENANALYST.md"), "nested rules")
            .expect("write nested rules");
        fs::write(
            nested.join(".openanalyst").join("instructions.md"),
            "nested instructions",
        )
    ...
    fn dedupes_identical_instruction_content_across_scopes() {
        ...
        fs::write(root.join("OPENANALYST.md"), "same rules\n\n").expect("write root");
        fs::write(nested.join("OPENANALYST.md"), "same rules\n").expect("write nested");
    ...
    fn displays_context_paths_compactly() {
        assert_eq!(
            display_context_path(Path::new("/tmp/project/.openanalyst/OPENANALYST.md")),
            "OPENANALYST.md"
        );
    }
    ...
    fn discover_with_git_includes_status_snapshot() {
        ...
        fs::write(root.join("OPENANALYST.md"), "rules").expect("write instructions");
        ...
        assert!(status.contains("?? OPENANALYST.md"));
    ...
    #[test]
    fn load_system_prompt_reads_openanalyst_files_and_config() {
        let root = temp_dir();
        fs::create_dir_all(root.join(".openanalyst")).expect("openanalyst dir");
        fs::write(root.join("OPENANALYST.md"), "Project rules").expect("write instructions");
        fs::write(
            root.join(".openanalyst").join("settings.json"),
        ...
        let original_openanalyst_home = std::env::var("OPENANALYST_CONFIG_HOME").ok();
        std::env::set_var("HOME", &root);
        std::env::set_var("OPENANALYST_CONFIG_HOME", root.join("missing-home"));
...
",
        ...
        if let Some(value) = original_openanalyst_home {
            std::env::set_var("OPENANALYST_CONFIG_HOME", value);
        } else {
    ...
    #[test]
    fn renders_openanalyst_style_sections_with_project_context() {
        let root = temp_dir();
        fs::create_dir_all(root.join(".openanalyst")).expect("openanalyst dir");
        fs::write(root.join("OPENANALYST.md"), "Project rules").expect("write OPENANALYST.md");
        fs::write(
            root.join(".openanalyst").join("settings.json"),
            r#"{"permissionMode":"acceptEdits"}"#,
        )
        ...
        assert!(prompt.contains("# OpenAnalyst instructions"));
        assert!(prompt.contains("Project rules"));
        assert!(prompt.contains("permissionMode"));
    ...
    #[test]
    fn discovers_dot_openanalyst_instructions_markdown() {
        let root = temp_dir();
        let nested = root.join("apps").join("api");
        fs::create_dir_all(nested.join(".openanalyst")).expect("nested openanalyst dir");
        fs::write(
        ...
        assert!(context
            .instruction_files
            .iter()
            .any(|file| file.path.ends_with(".openanalyst/instructions.md")));
        assert!(
    ...
    fn renders_instruction_file_metadata() {
        let rendered = render_instruction_files(&[ContextFile {
            path: PathBuf::from("/tmp/project/OPENANALYST.md"),
            content: "Project rules".to_string(),
        }]);
        assert!(rendered.contains("# OpenAnalyst instructions"));
