...
pub const OPENANALYST_SETTINGS_SCHEMA_NAME: &str = "SettingsSchema";
...
impl ConfigLoader {
    ...
    pub fn discover(&self) -> Vec<ConfigEntry> {
        let user_legacy_path = self.config_home.parent().map_or_else(
            || PathBuf::from(".openanalyst.json"),
            |parent| parent.join(".openanalyst.json"),
        );
        vec![
            ConfigEntry {
            ...
            ConfigEntry {
                ...
                path: self.cwd.join(".openanalyst.json"),
            },
            ConfigEntry {
                source: ConfigSource::Project,
                path: self.cwd.join(".openanalyst").join("settings.json"),
            },
            ConfigEntry {
                source: ConfigSource::Local,
                path: self.cwd.join(".openanalyst").join("settings.local.json"),
            },
        ]
    }
...
fn read_optional_json_object(
    ...
    let is_legacy_config = path.file_name().and_then(|name| name.to_str()) == Some(".openanalyst.json");
...
mod tests {
    use super::{
        ...
        OPENANALYST_SETTINGS_SCHEMA_NAME,
    };
    use crate::json::JsonValue;
    ...
    fn rejects_non_object_settings_files() {
        ...
        let home = root.join("home").join(".openanalyst");
    ...
    #[test]
    fn loads_and_merges_openanalyst_config_files_by_precedence() {
        let root = temp_dir();
        let cwd = root.join("project");
        let home = root.join("home").join(".openanalyst");
        fs::create_dir_all(cwd.join(".openanalyst")).expect("project config dir");
        fs::create_dir_all(&home).expect("home config dir");

        fs::write(
            home.parent().expect("home parent").join(".openanalyst.json"),
        ...
        fs::write(
            cwd.join(".openanalyst.json"),
            r#"{"model":"project-compat","env":{"B":"2"}}"#,
        )
        .expect("write project compat config");
        fs::write(
            cwd.join(".openanalyst").join("settings.json"),
            r#"{"env":{"C":"3"},"hooks":{"PostToolUse":["project"]},"mcpServers":{"project":{"command":"uvx","args":["project"]}}}"#,
        )
        .expect("write project settings");
        fs::write(
            cwd.join(".openanalyst").join("settings.local.json"),
        ...
        assert_eq!(OPENANALYST_SETTINGS_SCHEMA_NAME, "SettingsSchema");
