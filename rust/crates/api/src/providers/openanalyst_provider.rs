...
#[derive(Debug, Clone)]
pub struct OpenAnalystApiClient {
    http: reqwest::Client,
    auth: AuthSource,
    base_url: String,
    max_retries: u32,
...
fn resolve_saved_oauth_token_set(
    ...
    let client = OpenAnalystApiClient::from_auth(AuthSource::None).with_base_url(read_base_url());
...
fn read_auth_token() -> Option<String> {
    read_env_non_empty("OPENANALYST_AUTH_TOKEN")
        .ok()
...
pub fn read_base_url() -> String {
    std::env::var("OPENANALYST_BASE_URL")
...
impl Provider for OpenAnalystApiClient {
...
mod tests {
    ...
    use super::{
        ...
        resolve_startup_auth_source, AuthSource, OpenAnalystApiClient, OAuthTokenSet,
    };
    ...
    fn read_api_key_requires_non_empty_value() {
        let _guard = env_lock();
        std::env::set_var("OPENANALYST_AUTH_TOKEN", "");
        std::env::remove_var("OPENANALYST_API_KEY");
    ...
    fn read_api_key_prefers_api_key_env() {
        let _guard = env_lock();
        std::env::set_var("OPENANALYST_AUTH_TOKEN", "auth-token");
        std::env::set_var("OPENANALYST_API_KEY", "legacy-key");
        assert_eq!(
    ...
    fn backoff_doubles_until_maximum() {
        let client = OpenAnalystApiClient::new("test-key").with_retry_policy(
            3,
