...
const SLASH_COMMAND_SPECS: &[SlashCommandSpec] = &[
    ...
    SlashCommandSpec {
        name: "config",
        aliases: &[],
        summary: "Inspect OpenAnalyst config files or merged sections",
    ...
    SlashCommandSpec {
        name: "memory",
        aliases: &[],
        summary: "Inspect loaded OpenAnalyst instruction memory files",
        argument_hint: None,
        resume_supported: true,
    },
    SlashCommandSpec {
        name: "init",
        aliases: &[],
        summary: "Create a starter OPENANALYST.md for this repo",
        argument_hint: None,
        resume_supported: true,
    },
    SlashCommandSpec {
    ...
    SlashCommandSpec {
        ...
        summary: "Manage OpenAnalyst plugins",
        argument_hint: Some(
...
impl DefinitionSource {
    fn label(self) -> &'static str {
        match self {
            ...
            Self::ProjectOpenAnalyst => "Project (.openanalyst)",
            Self::UserCodexHome => "User ($CODEX_HOME)",
            Self::UserCodex => "User (~/.codex)",
            Self::UserOpenAnalyst => "User (~/.openanalyst)",
        }
    }
}
...
pub fn handle_commit_push_pr_slash_command(
    ...
    let body_path = write_temp_text_file("openanalyst-pr-body", "md", request.pr_body.trim())?;
...
fn discover_definition_roots(cwd: &Path, leaf: &str) -> Vec<(DefinitionSource, PathBuf)> {
    ...
    for ancestor in cwd.ancestors() {
        ...
        push_unique_root(
            &mut roots,
            DefinitionSource::ProjectOpenAnalyst,
            ancestor.join(".openanalyst").join(leaf),
        );
    }
    ...
    if let Some(home) = env::var_os("HOME").or_else(|| env::var_os("USERPROFILE")) {
        ...
        push_unique_root(
            &mut roots,
            DefinitionSource::UserOpenAnalyst,
            home.join(".openanalyst").join(leaf),
        );
    }
...
fn discover_skill_roots(cwd: &Path) -> Vec<SkillRoot> {
    ...
    for ancestor in cwd.ancestors() {
        ...
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::ProjectOpenAnalyst,
            ancestor.join(".openanalyst").join("skills"),
            SkillOrigin::SkillsDir,
        );
        push_unique_skill_root(
            &mut roots,
        ...
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::ProjectOpenAnalyst,
            ancestor.join(".openanalyst").join("commands"),
            SkillOrigin::LegacyCommandsDir,
        );
    }
    ...
    if let Some(home) = env::var_os("HOME").or_else(|| env::var_os("USERPROFILE")) {
        ...
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::UserOpenAnalyst,
            home.join(".openanalyst").join("skills"),
            SkillOrigin::SkillsDir,
        );
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::UserOpenAnalyst,
            home.join(".openanalyst").join("commands"),
            SkillOrigin::LegacyCommandsDir,
        );
    }
...
fn render_agents_usage(unexpected: Option<&str>) -> String {
    let mut lines = vec![
        ...
        "  Direct CLI       openanalyst agents".to_string(),
        "  Sources          .codex/agents, .openanalyst/agents, $CODEX_HOME/agents".to_string(),
    ];
...
fn render_skills_usage(unexpected: Option<&str>) -> String {
    let mut lines = vec![
        ...
        "  Direct CLI       openanalyst skills".to_string(),
        "  Sources          .codex/skills, .openanalyst/skills, legacy /commands".to_string(),
    ];
    if let Some(args) = unexpected {
        lines.push(format!("  Unexpected       {args}"));
    }
...
mod tests {
    ...
    fn init_git_repo(label: &str) -> PathBuf {
        ...
        run_command(&root, "git", &["config", "user.name", "OpenAnalyst Tests"]);
        run_command(&root, "git", &["config", "user.email", "openanalyst@example.com"]);
    ...
    fn write_external_plugin(root: &Path, name: &str, version: &str) {
        fs::create_dir_all(root.join(".openanalyst-plugin")).expect("manifest dir");
        fs::write(
            root.join(".openanalyst-plugin").join("plugin.json"),
            format!(
    ...
    fn lists_skills_from_project_and_user_roots() {
        ...
        let project_commands = workspace.join(".openanalyst").join("commands");
        ...
        let roots = vec![
            ...
            SkillRoot {
                source: DefinitionSource::ProjectOpenAnalyst,
        ...
        assert!(report.contains("Project (.openanalyst):"));
    ...
    fn agents_and_skills_usage_support_help_and_unexpected_args() {
        ...
        assert!(agents_help.contains("Direct CLI       openanalyst agents"));
